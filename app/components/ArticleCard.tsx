...
export default function ArticleCard({ article }: { article: { title: string; description: string; date: string; link: string; image: string } }) {
    return (
        <div className="group relative text-left bg-white/30 border border-gray-300 rounded-lg mb-6 shadow-md hover:shadow-lg transition-shadow duration-300 w-full max-w-xs min-h-[350px] flex flex-col justify-end overflow-hidden mx-auto">
            <Link href={article.link} className="block h-full w-full">
                ...
                <div className="p-6 flex-1 flex flex-col justify-end">
                    ...
                    <h3 className="absolute w-full text-2xl font-semibold mb-2 transition-transform duration-500 bottom-6 group-hover:-translate-y-32 pointer-events-none truncate">
                        {article.title}
                    </h3>
